...
#==============================================================================
# Variables
#STACK_NAME=$(awk -F "=" '/StackName_LambdaForSpotPlacementScore/ {print $2}' ../conf.ini | tr -d ' ')
STACK_NAME=$(get_config_value "StackName_LambdaForSpotPlacementScore")

#regions_string=$(awk -F "=" '/^Region_LambdaForSpotPlacementScore[[:space:]]*=[[:space:]]*/ {print $2}' ../conf.ini | tr -d ' ')

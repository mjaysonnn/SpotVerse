...
FILENAME="cloudformation_template_for_lambda_spot_placement_score.yaml"
STACK_NAME=$(awk -F "=" '/stack_for_lambda_for_spot_placement_score/ {print $2}' ../conf.ini | tr -d ' ')
DESIRED_STATUS="CREATE_COMPLETE"
DESIRED_STATUS_FOR_UPDATE="UPDATE_COMPLETE"

LAMBDA_CODE_DIRECTORY="lambda_codes/lambda_spot_placement_score_inserter"
LAMBDA_ZIP_FILE="lambda_spot_placement_score_inserter_$(printf "%04d" $((RANDOM % 10000))).zip"

regions_string=$(awk -F "=" '/^default_region_for_lambda_for_spot_placement_score[[:space:]]*=[[:space:]]*/ {print $2}' ../conf.ini | tr -d ' ')
...
# assuming python3 is in your PATH, adjust as necessary
PYTHON_CMD="/Users/mj/opt/anaconda3/envs/MultiCloudGalaxy/bin/python3"

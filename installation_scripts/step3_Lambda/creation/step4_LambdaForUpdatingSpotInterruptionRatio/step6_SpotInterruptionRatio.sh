...
#regions_string=$(awk -F "=" '/^Region_LambdaForInterruptionRatio[[:space:]]*=[[:space:]]*/ {print $2}' ../conf.ini | tr -d ' ')
regions_string=$(get_config_value "Region_LambdaForInterruptionRatio")

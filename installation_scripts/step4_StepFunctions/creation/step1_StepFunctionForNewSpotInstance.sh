...
update_conf_with_arn() {
  ...
  arn_name_variable="StateMachineArnForLambdaNewSpotInstance-$region"

  # Then use this variable in the subsequent commands
...
TEMPLATE_FILE="template_StepFunctionForNewSpotInstance.yaml"
#STACK_NAME=$(awk -F "=" '/StackName_StepFunctionForNewSpotInstance/ {print $2}' ../conf.ini | tr -d ' ')
STACK_NAME=$(get_config_value "StackName_StepFunctionForNewSpotInstance")

STATE_MACHINE_NAME="StepFunctionForNewSpotInstance"
